...
const FiveDayAverageGraph = () => {
  ...
  useEffect(() => {
    const fetchWeights = async () => {
      ...
      if (weights) {
        ...
        const rollingAverageData = calculateRollingAverage(weights, 5);
  ...
  let htmlContent = `
    ...
    Highcharts.chart('container', {
      ...
      title: {
        text: '5 Day Rolling Average Weight'
      },
      xAxis: {
  ...
  if (averageData.length === 0) {
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
        <Text>Not enough data to calculate rolling average. Need at least 5 data points.</Text>
      </View>
    );
  }
