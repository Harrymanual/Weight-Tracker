...
const ThreeDayAverageGraph = () => {
  ...
  useEffect(() => {
    const fetchWeights = async () => {
      ...
      if (weights) {
        ...
        const rollingAverageData = calculateRollingAverage(weights, 3);
  ...
  let htmlContent = `
    ...
    Highcharts.chart('container', {
      ...
      title: {
        text: '3 Day Rolling Average Weight'
      },
      xAxis: {
  ...
  if (averageData.length === 0) {
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
        <Text>Not enough data to calculate rolling average. Need at least 3 data points.</Text>
      </View>
    );
  }
